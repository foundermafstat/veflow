...
export const BaseNode = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      ...
      // Handle selected state - React Flow adds 'selected' class to parent
      "data-[selected=true]:ring-2 data-[selected=true]:ring-primary data-[selected=true]:shadow-lg",
      className,
    )}
    tabIndex={0}
    {...props}
...
export const BaseNodeHeader = forwardRef<HTMLElement, HTMLAttributes<HTMLElement>>(({ className, ...props }, ref) => (
  <header
    ...
    {...props}
    className={cn(
      "mx-0 my-0 -mb-1 flex flex-row items-center justify-between gap-2 px-3 py-2 border-b",
